...
all_sequences = {
    ...
    'PETS09-S2L2': Sequence('PETS09-S2L2', nframes=436, fps=7, width=768, height=576),
}

parser = argparse.ArgumentParser(description='2D tracker test.')
parser.add_argument('--traindir', nargs='?', default='/home/stefan/data/MOTChallenge/3DMOT2015/test/',
...
all_threshs = [50]#range(10,201,10)
all_motas = []
all_motps = []

for dist_thresh in all_threshs:
    ...
    if args.eval:
        ...
        eval_hypos = []
    m = Munkres()
    #dist_thresh = 76 #pixel #TODO: dependent on resolution
        ...
        for each_tracker in track_list:
            ...
            dist_matrix_line = np.array([euclidean(each_tracker.KF.x[::2],
                                            (curr_dets[i][2]+curr_dets[i][4]/2.,
                                             curr_dets[i][3]+curr_dets[i][5]/2.)) for i in range(len(curr_dets))])
    ...
    if args.eval:
        ...
        with open(pjoin(seq_dir, 'gt/gt.txt'), 'r') as gt_file:
            ...
            for gt_line in gt_file:
                ...
                if (len(one_gt) == 10):
                    ...
                    one_gt[2] = int(round(one_gt[2]))
                    one_gt[3] = int(round(one_gt[3]))
        ...
        evaluator = MOTEvaluation(eval_groundtruth, eval_hypotheses, use3Dinput=True,distance_threshold=50)
        evaluator.evaluate()
        print('===Evaluation results===')
        print('MOTA:', evaluator.getMOTA())
        print('MOTP:', evaluator.getMOTP())
        all_motas.append(evaluator.getMOTA())
        all_motps.append(evaluator.getMOTP())
...
if args.eval:
    ...
    print(all_motas)
    print(all_motps)
    print ('Max MOTA: ', max(all_motas), 'with thresh: ', all_threshs[all_motas.index(max(all_motas))], '(MOTP: ', all_motps[all_motas.index(max(all_motas))], ')')
    print ('Min MOTP: ', min(all_motps), 'with thresh: ', all_threshs[all_motps.index(min(all_motps))], '(MOTA: ', all_motas[all_motps.index(min(all_motps))], ')')

    f, axarr = plt.subplots(2, sharex=True)
    axarr[0].plot(all_threshs, all_motas)
    axarr[0].set_title('MOTA')
    axarr[1].plot(all_threshs, all_motps)
    axarr[1].set_title('MOTP')
    plt.show()